...
<li>
    ...
    <br></br>
                   ...
                   int Product_id=Integer.parseInt(request.getParameter("product_id"));
                    System.out.println("pidkkfgxhnfgyjhn "+Product_id);
                    String color_id=request.getParameter("color_id");
                    System.out.println("cidkk "+color_id);
            	    Connection con = Db.Condb();
                	String sql1="select t.PRODUCT_ID,p.PRODUCT_NAME,p.MSRP,p.DISCOUNTS,t.file_name,p.SHORT_DESCRIPTION,p.DETAILED_DESCRIPTION,c.color,p.SUPPLIER_ID from image t inner join products p  on t.PRODUCT_ID=p.PRODUCT_ID inner join color c on t.PRODUCT_ID=c.PRODUCT_ID where p.PRODUCT_ID=? and t.COLOR=?";
                	PreparedStatement ps=con.prepareStatement(sql1);
                	ps.setInt(1,Product_id);
                	ps.setString(2,color_id);
                ...
                <img class="parentimage" src="Image6?product_id=<%=Product_id%>&filename=<%=filename%>" height="420" width="420" height="320" width="320" alt="" />            
                <img class="parentimageLarge" src="Image6?product_id=<%=Product_id%>&filename=<%=filename%>" height="420" width="420" height="320" width="320" alt="" />  
                 ...
                 <%  
                       ...
                       String sql2="select FILE_NAME from image   where PRODUCT_ID=? and FILE_NAME!=? and COLOR=?";
                   	 PreparedStatement ps1=con.prepareStatement(sql2);
                   	ps1.setInt(1,Product_id);
                   	ps1.setString(2,filename);
                   	ps1.setString(3, color_id);
                      ...
                      %>
                        ...
                        <td >   &nbsp;&nbsp;&nbsp;    <a href="singleproduct.jsp?product_id=<%=Product_id%>&filename=<%=str%>"><img src="Image_single?product_id=<%=Product_id%>&filename=<%=str%>" alt=""  ></a></td>
                                      <% 
                        	}%> 
                                   </tr>
                                    </table>   
                                ...
                                <div class="product-inner">
                                    ...
                                    <form action="Cart" class="cart" method="post">
                                      <!--   <div class="quantity">
                                        ...
                                        <button class="add_to_cart_button" type="submit" name="p" value="cart">Add to cart</button>
                                        <button class="add_to_cart_button" type="submit" name="p" value="buy">buy now</button>
                                    </form>   
                	...
                	int i1=1;%>
                      ...
                      <span >  <table><%} %>
                        ...
                        <td style="height:4%;width:15%"><a href="Single_product_color.jsp?product_id=<%=Product_id%>&color_id=<%=rs8.getString("COLOR_ID")%>&Product_Name=<%=PRODUCT_NAME%>&color=<%=rs8.getString("COLOR")%>"><%=rs8.getString("COLOR")%></a></td>
    ...
    %></td> </tr>
                         ...
                         PRODUCT_NAME=request.getParameter("Product_Name");   
                          int i=0;
                    System.out.println("pid in single product"+PRODUCT_NAME);
                                        ...
                                        <input type="hidden" name="shor_des" value="<%=rs3.getString("SHORT_DESCRIPTION")%>"/>
                                         <input type="hidden" name="price" value="<%=rs3.getDouble("SALES_PRICE")%>"/>
                                          <input type="hidden" name="name" value="<%=PRODUCT_NAME%>"/>
                                           <input type="hidden" name="seller" value="<%=rs3.getString("SUPPLIER_ID")%>"/></td>
